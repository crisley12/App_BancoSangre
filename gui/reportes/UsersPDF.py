...
for user in users:
    email = user['email']
    #paciente_id = str(user['paciente_id'])  # Convertir ObjectId a cadena
    #role_id = str(user['role_id'])  # Convertir ObjectId a cadena
    user_id = str(user['_id'])

    pdf.cell(12, 10, str(valor_celda), 1, 0, 'C', 0)
    pdf.cell(83, 10, email, 1, 0, 'C', 1)
    #pdf.cell(45, 10, paciente_id, 1, 0, 'C', 1)
    #pdf.cell(35, 10, role_id, 1, 0, 'C', 1)
    pdf.cell(95, 10, str(user_id), 1, 1, 'C', 1)
